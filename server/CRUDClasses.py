...
class MenuCRUD:
    ...
    async def create_menu(self, data):
        ...
        await self.cache_rep.set(f'menu_{menu.id}', menu)
    ...
    async def get_all_menus(self):
        ...
        await self.cache_rep.set('all_menus', [menu for menu in menus])
        return menus
    ...
    async def get_menu(self, menu_id):
        ...
        await self.cache_rep.set(f'menu_{menu_id}', menu)
        return menu

    async def update_menu(self, menu_id, data):
        try:
            updated_menu = await self.menu_rep.update_menu(menu_id, data)
            await self.cache_rep.set(f'menu_{menu_id}', updated_menu)
...
class SubmenuCRUD:
    ...
    async def create_submenu(self, menu_id, data):
        try:
            ...
            await self.cache_rep.set(f'submenu_{submenu.id}', submenu)
    ...
    async def get_submenu(self, submenu_id):
        ...
        await self.cache_rep.set(f'submenu_{submenu_id}', submenu)
        return submenu
    ...
    async def get_all_submenus(self, menu_id):
        ...
        await self.cache_rep.set(f'menu_submenus_{menu_id}', [submenu for submenu in submenus])
        return submenus

    async def update_submenu(self, submenu_id, data):
        try:
            updated_submenu = await self.submenu_rep.update_submenu(submenu_id, data)
            await self.cache_rep.set(f'submenu_{submenu_id}', updated_submenu)
...
class DishCRUD:
    ...
    async def create_dish(self, submenu_id, data):
        try:
            ...
            await self.cache_rep.set(f'dish_{dish.id}', dish)
    ...
    async def get_dish(self, dish_id):
        ...
        await self.cache_rep.set(f'dish_{dish_id}', dish)
        return dish
    ...
    async def get_all_dishes(self, submenu_id):
        ...
        await self.cache_rep.set(f'submenu_dishes_{submenu_id}', [dish for dish in dishes])
        return dishes

    async def update_dish(self, dish_id, data):
        try:
            updated_dish = await self.dish_rep.update_dish(dish_id, data)
            await self.cache_rep.set(f'dish_{dish_id}', updated_dish)
